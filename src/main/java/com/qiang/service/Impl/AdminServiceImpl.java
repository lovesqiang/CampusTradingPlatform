...
public class AdminServiceImpl implements AdminService {
    ...
    public Admin aLogin(String name, String pwd) {
        ...
        example.createCriteria().andAdminAccountEqualTo(name);     //追加条件a_name=name

        //select * from admin where a_name=name;
        List<Admin> list = adminMapper.selectByExample(example);	//调用数据持久层的方法
        ...
        if(list.size()>0){
            ...
            if(mipwd.equals(admin.getAdminPassword())){    //判断用户密码和数据库存储的密码是否一致
                return admin;
            }
        }
